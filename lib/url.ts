...
export function normalizeNotesInput(text: string): NormalizeResult {
  ...
  if (/^[0123456M-]+$/.test(trimmed) && trimmed.includes("-")) {
    return validateCompact(trimmed);
  }
  if (/^[0123456M]+$/.test(trimmed) && trimmed.length % 4 === 0 && !trimmed.includes("\n")) {
    return validateCompact(trimmed);
  }
  ...
  for (const mt of measureTexts) {
    ...
    for (const line of lines) {
      ...
      // 未対応のノート種 (K/L/F など) は 0 に置換 (5/6=空打ちは本アプリ独自の拡張)
      rows.push(line.toUpperCase().replace(/[^0123456M]/g, "0"));
    }
